...
Открывается в любом браузере. Для входа используйте кнопку «Войти через Telegram» — авторизация через официальный Telegram OAuth.
...
<b>Управление:</b>
• <b>📊 Статистика</b> — просмотры, поиски, оценки за неделю
• <b>👥 Пользователи</b> — список всех пользователей, профили, 🔥 = Premium
• <b>🚀 Деплой</b> — статус и пересборка Vercel
• <b>📢 Уведомление</b> — рассылка всем пользователям (с кнопкой 🏠)
• <b>🧹 Очистить</b> — сброс сессий / полная очистка статистики
• <b>🔥 Premium</b> — купить/проверить свой Premium статус
...
• 🎉 Новом пользователе (сайт/бот/приложение)
• 🔥 Новом Premium-покупателе
• 💸 Запросе на возврат звёзд (кнопки ✅/❌)
...
` + buildUserHelpText();
    ...
    if (refundApproveMatch) {
      ...
      try {
        ...
        // from this user. We iterate in REVERSE to get the latest one first.
        // Transaction format: source.user.id (NOT source.user_id)
        var userTxn = null;
...
async function registerBotCommands() {
  ...
  try {
    ...
    // IMPORTANT: include secret_token if WEBHOOK_SECRET is set, otherwise
    // we'd overwrite the webhook and REMOVE the secret_token that the user
    // configured. This was causing 401 Unauthorized errors because Telegram
    // stopped sending the secret header after we overwrote the webhook.
    try {
      var webhookParams = {
      ...
      if (process.env.WEBHOOK_SECRET) {
        webhookParams.secret_token = process.env.WEBHOOK_SECRET;
      }
      await tg('setWebhook', webhookParams);
      console.log('[bot] Webhook updated with full allowed_updates' + (process.env.WEBHOOK_SECRET ? ' + secret_token' : ''));
    } catch (e) {
